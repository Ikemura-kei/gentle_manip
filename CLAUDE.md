...
- **Task / dataset naming — HARD RULE:** `{single,multi}_{task}_{object}_{soft,rigid,real}`.
  ...
  - Examples: `single_lift_mushroom_soft`, `single_lift_mushroom_rigid`, `single_lift_mushroom_real`.
  - **Violations to correct:** `mushroom_lift` → `single_lift_mushroom_real`; `mushroom_soft_batched` → `single_lift_mushroom_soft`; do NOT append obs/dataset-size qualifiers to the task name (e.g. `_pcd_wide`, `_1k`, `_n150` belong in the run description or experiment name, not the task key). The task name is a canonical identifier for the TASK + OBJECT + MODALITY-AGNOSTIC source; everything else is experiment metadata.
