...
def plan_width_grasp(obj, mesh, *, E=None, density=None, mu=None, dr=None, hold_frac: float = 1.0,
                     w_align=None, x0=None, sigma: float = 0.3, maxfevals: int = 200,
    ...
    aln = {} if w_align is None else {"w_align": w_align}         # None -> module default W_ALIGN
    ...
    # ── ROUND 2: pick the top DISTINCT poses from round 1, then optimize ONLY the 1-D WIDTH for each
    # (same filter + FEM). A 1-D width scan is trivial and reliable, unlike the noisy 6-DOF search, so
    # this robustly polishes each promising pose to its best gentle-holdable width.
    if refine and feasible:
        picked = _distinct_poses(feasible, n_refine, axis_deg=25.0, center_thr=0.12 * ext)
        for pi, (c, th, ph) in enumerate(picked):
...
def _axis_seeds(n_starts: int, seed: int):
    ...
    canon = [(np.pi / 2, 0.0), (np.pi / 2, np.pi / 2), (0.05, 0.0), (np.pi / 3, np.pi / 4),
             (np.pi / 2, np.pi / 4), (np.pi / 3, 1.25 * np.pi), (0.05, np.pi / 2), (2 * np.pi / 3, 0.0)]
