...
class ActionPipeline:
    ...
    def _process_absolute(self, clipped: np.ndarray) -> np.ndarray:
        ...
        if getattr(self.cfg, "gripper_delta", False):
            # DELTA gripper, ABSOLUTE pose. Scale factor ONLY, no affine offset (B10): a per-step
            # CHANGE the backend accumulates onto its running target (seeded from the MEASURED
            # width at reset), exactly as delta mode does.
