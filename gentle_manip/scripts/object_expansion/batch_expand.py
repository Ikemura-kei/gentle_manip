...
def process_one(cand: dict, *, n_episodes: int, n_envs: int, min_success: float,
                material: str, skip_collect: bool, max_attempts: int = 0) -> None:
    ...
    if max_attempts:                      # bound a low-yield object instead of grinding to n_episodes
...
def main() -> None:
    ...
    ap.add_argument("--max-attempts", type=int, default=0,
                    help="cap collection attempts per object (0 = off, grind to --n-episodes)")
    ...
    for cand in batch:
        process_one(cand, n_episodes=args.n_episodes, n_envs=args.n_envs,
                   ...
                   skip_collect=args.skip_collect, max_attempts=args.max_attempts)
    print("\nBatch done.")
