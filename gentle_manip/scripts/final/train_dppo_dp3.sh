...
# SEED=${SEED:-42}
# ── Round 3 (2026-09-06 night, run after qzhek): SAME as round 2 except: offsets +-6 mm (BC aug + consistency view),
#    consistency on 30 % of the batch, hold tail K=20 (= the frozen collector's hold), 1200 epochs. ──
DATASET=${DATASET:-single_lift_tofu_sim2real_v1_tail20}   # dataset/dppo/<DATASET>/{train,val,normalization}.npz; log-dir leaf
EXPERIMENT=${EXPERIMENT:-single_lift_tofu_soft_abs_action_armfocus_7d_realws}   # snapshotted to <run>/config/
EPOCHS=${EPOCHS:-1200}          # round 3; ~3000 x 29648 / N_train_steps, clamped [800, 3000]; best checkpoints were 750-1000 so far
PAIRED_W=${PAIRED_W:-0.5}       # real-vs-sim paired encoder term; 0 = off
PC_AUG=${PC_AUG:-d435i_noise_train}   # train-time cloud sensor noise + leaked residue (p=0.5), configs/augmentation/<name>.yaml; "" = off
CLOUD_OFFSET=${CLOUD_OFFSET:-0.006}   # per-sample rigid cloud offset U(+-m) on the BC clouds; 0 = off  (round 2: 0.008)
CONS_W=${CONS_W:-0.3}           # clean-vs-perturbed encoder consistency weight; 0 = off
CONS_FRAC=${CONS_FRAC:-0.3}     # fraction of each batch the consistency term uses (round 2: 0.5)
CONS_AUG=${CONS_AUG:-d435i_noise_strong}   # the perturbed view's augmentation yaml
CONS_OFFSET=${CONS_OFFSET:-0.006}          # rigid offset U(+-m) on the perturbed view (round 2: 0.012)
SEED=${SEED:-42}
