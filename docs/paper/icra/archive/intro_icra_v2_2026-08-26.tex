...
grasp forces. What such data must supply is \emph{diversity}. Lin \emph{et al.}
\cite{lin2025datascaling} show that policy generalisation follows a power law in the number of
environments and objects, and that once demonstrations per object pass a threshold, further
demonstrations add little; diversity, not count, is what buys generalisation. Precision
sharpens the requirement further: for high-precision tasks the number of demonstrations needed
grows super-exponentially as the tolerance tightens, $\log N \propto 1/(P-c)$, where the limit
precision $c$ is a property of the whole system including its expert~\cite{curseofprecision}.
Diversity is also expensive to obtain by demonstration, because a demonstration is recorded
against one item in one pose and cannot be transported to another. Automatic data generation
addresses part of this: MimicGen~\cite{mandlekar2023mimicgen} multiplies a few hundred human
demonstrations into tens of thousands by adapting them to new object placements. Such
transforms are geometric, however, and cannot express variation in material properties. That
matters here, because stiffness is one of the dimensions the correct grasp depends on, and it
is a dimension no real dataset can cover at all: a single physical specimen cannot be presented
at several stiffnesses. We see the shortfall directly in our own system. A policy trained on 50
real demonstrations does not degrade uniformly on hardware but fails in a structured way,
...
use it as a \emph{planning objective} rather than a sensed signal. Finite-element simulation
has been used to \emph{evaluate} grasps on deformable objects, notably
DefGraspSim~\cite{defgraspsim}, which measures stress and deformation outcomes for parallel-jaw
grasps across a large object set. We use it instead to \emph{search}. A stress-aware planner
...
gap~\cite{simrealcotrain}.
\textbf{[TODO: headline real-robot number + \# objects, stated here as DextrAH does.]}
...
\begin{itemize}
  ...
  \item An autonomous demonstration generator that produces gentle, successful grasps across
        randomised object pose, size, shape, and material with no human demonstration. Unlike
        data-generation systems that adapt existing human demonstrations to new object
        placements~\cite{mandlekar2023mimicgen}, it plans each grasp from scratch against a
        physics objective, and so covers material variation that a geometric trajectory
        transform cannot express and a real dataset cannot contain.
