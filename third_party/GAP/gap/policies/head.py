...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        feature_dim: Dim of global conditioning applied with FiLM 
    ...
    def forward(self, 
        ...
        for idx, (resnet, resnet2, downsample) in enumerate(self.down_modules):
            ...
            h.append(x)
            # print("the h.append() shape is:", x.shape)
            x = downsample(x)
    ...
    def get_action(self, x):
        # x: B,T,E
        B, T, E = x.shape
        # inference uses only the last feature as condition
        conditional_feature = x[:,-1]
        noisy_action = torch.randn((B, self.horizon, self.action_dim)).to(x.device)
        n_action = noisy_action
        # print("the x shape is:", x.shape)
        for k in self.noise_scheduler.timesteps:
            noise_pred = self.forward(n_action, k, global_cond=conditional_feature)
...
class DeterministicMLP(nn.Module):
    ...
    def forward(self, feature):
        # (-1, E)
        x = self.dmm(feature) # (-1, horizon*dim)
        x = x.view(-1,self.horizon,self.action_dim)
    ...
    def get_action(self, x):
        ...
        B, T, E = x.shape
        # inference uses only the last feature as condition
        last_feature = x[:,-1] # (Batch=1,E)
        pred_action = self(last_feature) # (Batch=1,horizon,dim)
        return pred_action
    ...
    def loss_fn(self, obs_feature, gt_action, reduction="mean"):
        ...
        """
        # (B,T,E) -> (B*T,E)
        B,T,E = obs_feature.shape
        feature = obs_feature.view(-1, E)
        pred = self(feature)
        pred_actions = pred.view(B,T,*pred.shape[-2:])
        pred_actions = pred_actions.squeeze()
        loss = nn.functional.mse_loss(pred_actions, gt_action, reduction=reduction)
        return loss
...
class TransformerForDiffusion(ModuleAttrMixin):
    ...
    def get_action(self, x):
        ...
        # print("the x shape is:", x.shape)
        for k in self.noise_scheduler.timesteps:
            noise_pred = self.forward(n_action, k, global_cond=x)
            n_action = self.noise_scheduler.step(
                model_output=noise_pred,
                timestep=k,
                sample=n_action
    ...
    def loss_fn(self, obs_feature, gt_action, reduction="mean"):
        ...
        # print("the noisy_actions shape is:", noisy_actions.shape)
        # print("the obs_feature shape is:", obs_feature.shape)
        noise_pred = self(noisy_actions, timesteps, global_cond=obs_feature)
        loss = nn.functional.mse_loss(noise_pred, noise, reduction=reduction)
        return loss
