...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        qkv = output.reshape(B, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
...
def drop_path(
    ...
    """
    # print("the drop_prob is:", drop_prob)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        for layer_idx, (att_norm, att, ff_norm, ff) in enumerate(self.layers):
            ...
            if mask is not None:
                output = att(att_norm(x),mask)
                x = x + drop_path(output)
            elif self.mask is not None:
                output = att(att_norm(x),mask)
                x = x + drop_path(output)
            else:  # no masking, just use full attention
                output = att(att_norm(x))
                x = x + drop_path(output)
...
if __name__ == '__main__':
    temporal_position_encoding_fn = SinusoidalPositionEncoding(6)
    # temporal = TransformerDecoder(
    #     input_size=512,
    #     num_layers=4,
    #     num_heads=6,
    #     head_output_size=64,
    ...
    a = torch.rand((16, 5, 2, 6))
    pos_emb = temporal_position_encoding_fn(a)
    print(pos_emb)
    # print("the pos_emb shape is:", pos_emb.shape)
    # sh = a.shape
    # a = a + pos_emb.unsqueeze(1)  # (B, T, num_modality, E)
    # temporal.compute_mask(a.shape)
    # print("the mask shape:", temporal.mask.shape)
    # a = torch.cat([a[:,:,0], a[:,:,1]], dim=-2) # N, T * num_modality, E

    # print("the joint x shape is:", a.shape)
    # a = temporal(a)

    # a = a.reshape(*sh)  # N,T,num_modality,E
    # a = a[:,:,-1]
    # print(a.shape)
